...
/**
 * 通过统一身份认证SDK2.0进行校验 (内部接口）
 * @param success 成功时的回调函数名称
 * @param fail 失败时的回调函数名称
 * @param username 用户名
 * @param password 密码
 */
RuixinApi.prototype.iamAuth =  function(params) {
	this.callNative("iamAuth", params);
};
 ...
 * 梦想云专用 -- 语音转文字开始
 */
RuixinApi.prototype.startTalking = function(params)
{
    this.callNative("startTalking",params);
}
                            
/**
 * 梦想云专用 -- 语音转文字结束
 */
RuixinApi.prototype.endTalking = function(params)
{
    this.callNative("endTalking",params);
}
